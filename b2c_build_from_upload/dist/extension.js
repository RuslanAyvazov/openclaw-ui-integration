...
async function controlToken(config) {
...
async function backendRequest(config, route, body, timeoutMs = 120_000) {
    ...
    if (!response.ok) {
        throw new B2cToolError("BACKEND_REQUEST_FAILED", String(payload.error || `Backend ответил ${response.status}.`));
    }
    return payload;
}
export async function listAccessibleDatamarts(config, runtime) {
    if (!runtime.agentId) {
        throw new B2cToolError("AGENT_CONTEXT_MISSING", "OpenClaw не передал agentId.");
    }
    return backendRequest(config, "/api/internal/openclaw/catalog", { agentId: runtime.agentId });
}
async function writeRepositoryCopy(repositoryDir, contents) {
...
async function uploadInputs(workspaceDir, uploadPath) {
    const relativeUploadPath = normalizedRelativePath(uploadPath);
    if (!UPLOAD_PATH.test(relativeUploadPath)) {
        throw new B2cToolError("INVALID_UPLOAD_PATH", "uploadPath должен иметь вид uploads/package-<уникальное-имя>.");
    }
...
async function runProcess(command, args, cwd, timeoutSeconds, signal) {
    return new Promise((resolve, reject) => {
        ...
        const stop = () => { if (child.exitCode === null)
            child.kill("SIGTERM"); };
...
async function collectRepository(repositoryDir) {
    const result = {};
    let total = 0;
...
export async function updateMart(request, config, runtime, signal) {
    if (!runtime.agentId || !runtime.workspaceDir) {
        throw new B2cToolError("AGENT_CONTEXT_MISSING", "OpenClaw не передал agentId или workspaceDir текущего агента.");
    }
    ...
    const exported = await backendRequest(config, "/api/internal/openclaw/repository", {
    ...
    for (const sqlFile of inputs.sql)
        args.push("--sql-file", sqlFile);
    const processResult = await runProcess("bash", args, jobDir, config.timeoutSeconds || 900, signal);
    if (processResult.code !== 0 && processResult.code !== 3) {
        throw new B2cToolError("EXTENSION_BUILD_FAILED", (processResult.stderr || processResult.stdout || `Скрипт завершился с кодом ${processResult.code}.`).trim());
    }
    ...
    const published = await backendRequest(config, "/api/internal/openclaw/repository/import-branch", {
        agentId: runtime.agentId,
        workspace,
        datamart,
        branch,
