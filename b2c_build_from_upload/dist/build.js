...
async function requireRegularFile(filePath, root) {
...
export async function resolveBuildInputs(workspaceDir, uploadId) {
    if (!UPLOAD_ID_PATTERN.test(uploadId)) {
        throw new B2cBuildError("INVALID_UPLOAD_ID", "uploadId должен содержать только латинские буквы, цифры, дефис и подчёркивание.");
    }
    ...
    const selectedXlsx = exactS2tNames.length === 1
        ...
        : xlsxNames.length === 1
            ...
            : undefined;
    if (!selectedXlsx) {
        throw new B2cBuildError("S2T_FILE_NOT_UNIQUE", "В пакете должен быть один файл S2T.xlsx. Если имя другое, XLSX-файл всё равно должен быть единственным.");
    }
    ...
    if (sqlNames.length === 0) {
        throw new B2cBuildError("SQL_FILES_NOT_FOUND", "Не найдены файлы .sql или .txt, в имени которых есть DML_inc или DML_arc.");
    }
    const s2tFile = await requireRegularFile(path.join(uploadDir, selectedXlsx), uploadDir);
    const sqlFiles = await Promise.all(sqlNames.map((name) => requireRegularFile(path.join(uploadDir, name), uploadDir)));
    return {
...
async function runProcess(command, args, cwd, timeoutSeconds, maxLogBytes, signal) {
...
async function listGeneratedFiles(rootDir, workspaceDir) {
    const result = [];
...
export async function executeBuild(request, config, runtime, signal) {
    if (!runtime.workspaceDir) {
        throw new B2cBuildError("WORKSPACE_CONTEXT_MISSING", "OpenClaw не передал workspace текущего агента. Инструмент нельзя запускать вне контекста агента.");
    }
    ...
    const scriptPath = await requireRegularFile(path.join(skillRoot, "scripts", "run_from_files.sh"), skillRoot);
    ...
    args.push("--dml-json", dmlJson, "--output-json", contextConfig, "--mart-dir", martDirectory);
    ...
    const processResult = await runProcess("bash", args, inputs.uploadDir, timeoutSeconds, maxLogBytes, signal).catch((error) => {
    ...
    if (processResult.timedOut) {
        throw new B2cBuildError("BUILD_TIMEOUT", `Сборка превысила ограничение ${timeoutSeconds} секунд. Последний вывод: ${processResult.stderr || processResult.stdout}`);
    }
    if (processResult.exitCode !== 0) {
        throw new B2cBuildError("BUILD_SCRIPT_FAILED", `run_from_files.sh завершился с кодом ${processResult.exitCode ?? "нет"}. ${processResult.stderr || processResult.stdout}`);
    }
