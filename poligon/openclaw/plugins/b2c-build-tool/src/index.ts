...
import {
  B2cToolError,
  listAccessibleDatamarts,
  updateMart,
} from "./extension.js";
import type { UpdateRequest } from "./extension.js";
...
const updateParameters = Type.Object(
  {
    ...
    storage: Type.Optional(
      Type.Union([Type.Literal("iceberg"), Type.Literal("parquet")], { default: "iceberg" }),
    ),
  },
  { additionalProperties: false },
);
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      description:
        "Returns only workspaces and datamarts available to the current OpenClaw user. "
    ...
    tool({
      ...
      description:
        "Copies an accessible datamart branch into the current agent workspace, validates format 2.0, "
      ...
      factory: ({ config, toolContext }) => ({
        ...
        execute: async (_callId, params, signal, onUpdate) => {
          const request = params as UpdateRequest;
          onUpdate?.({
            content: [],
